...
- Gallery clips (GALLERY_CLIPS in constants.ts): the owner's own phone videos, real footage and not
  generated, shown at the top of the gallery's Recent work list through the same Loop component
  (muted, looping, on screen only, poster first). They document a job, so they may move however
  the phone moved. Keep them to a few and keep each under about 2 MB.
...
- Reviews are real Google reviews only (`src/lib/reviews.ts`, generated by `scripts/fetch-reviews.mjs` with `GOOGLE_PLACES_API_KEY`). The rating and count print with an "as of" date and a link to the listing; quotes are verbatim, attributed to Google, with first name and last initial. Refresh the file before a launch or when the count on the site looks stale. Never put the Google rating into JSON-LD (Google does not allow third-party aggregate ratings there).
- Ceramic coatings are Nasiol only (the owner does not apply 3M or 3D coatings). "Certified in Nasiol, 3M, and 3D" stays true as a products statement; any sentence about the coating itself says Nasiol.
...
All web photos live in `public/photos/*.webp`, exported from the owner's library at 1600px long side. Studio shots: `hero-foam` (GT3 in snow foam), `shop-gt3` (GT3 clean under the hex lights). Living-photo clips generated from `hero-foam`, `shop-gt3` and `boat-side-wide` live in `public/video/` (see Motion). Mobile rig: `truck-bubbles` (the black lettered van, on the shop card, cropped 4:3) and `trailer` (gallery). The van and rig-lot photos were removed from `public/` on 2026-09-04 (van: its wrap prints a review count the site does not claim; rig-lot: a phone screenshot with a black band) and stay recoverable from commit 26f8a0a. `boat-side` was replaced by the 16:9 crop `boat-side-wide` so its clip and poster share one frame. City pages open on `stelvio`, a driveway job. Service index and service page heroes use the owner's picks `svc-exterior`, `svc-interior`, `svc-paint`, `svc-ceramic` (cropped 4:5) and `svc-marine` (his after/before bow composite, labels kept). Owner batch of 2026-09-05 added the studio and RV shots (`challenger`, `red-coupe-studio`, `truck-white-studio`, `suv-black-studio`, `merc-interior-red`, `boxster`, `truck-cabin-grey`, `suv-cargo-clean`, `rv-eagle`, `rv-north-point`, `rv-roof`, `speedboat`), the `ba-engine-*` pair (landscape, spans two columns) and the two gallery clips. Real before/after pairs: `ba-cargo-*`, `ba-trunk-*`, `ba-seats-*`, `ba-cabin-*`. Everything else is named by vehicle. Use `asset()` from `src/lib/asset.ts` for every src so the GitHub Pages base path keeps working.
